<?php
/**
 * File Name BookClubsPostType.php
 * @package WordPress
 ...
 * @updated 00.00.13
 **/
$BookClubsPostType = new BookClubsPostType();
class BookClubsPostType {
		...
		$this->registered = register__post_type( array(
			'post_type' => array(
				'labels' => array(
					...
					'search_items' => __( 'Search Book Clubs', 'childtheme' ),
					'not_found' => __( 'No Book Clubs found', 'childtheme' ),
					'not_found_in_trash' => __( 'No Book Clubs found in Trash', 'childtheme' ), 
					'parent_item_colon' => '',
					'menu_name' => __( 'Book Clubs', 'childtheme' )
				),
				...
				// 'menu_position' => null,
				// 'menu_icon' => get_stylesheet_directory_uri() . "/addons/PostTypes/images/book-club-16x16.png", // is set in class construct
				...
				// 'taxonomies' => array('post_tag', 'book-club-tax-hierarchal'), // array of registered taxonomies
				...
				'query_var' => 'book-club', // This goes to the WP_Query schema
				'can_export' => true,
	...
	function edit_columns( $columns ) {
		...
		$columns['email'] = __( 'Email', 'childtheme' );
		$columns['address'] = __( 'Address', 'childtheme' );
		$columns['desc'] = __( 'Description', 'childtheme' );
	...
	function custom_columns( $column ) {
		...
		if ( $post->post_type == $this->registered->_post_type ) {
			...
			switch ( $column ) {
				...
				case "email":
					if ( get_field('_book_club__email', $post->ID) ) {
						echo get_field('_book_club__email', $post->ID);
					}
					break;
				case "desc" : 
					if ( get_post_meta( $post->ID, '_book_club__desc', true ) ) {
						echo wpautop( substr( get_post_meta( $post->ID, '_book_club__desc', true ), 0, 50 ) . "..." );
					}
					break;
				case "address" :
					if ( ( isset( $post->location->city ) and ! empty( $post->location->city ) ) OR ( isset( $post->location->state_long ) and ! empty( $post->location->state_long ) ) ) {
						echo $post->location->city . " " . $post->location->state_long;
					}
					break;
